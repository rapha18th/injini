...
**Dataset export and remote sync**, both from the Machines screen: a live counter ("Dataset: N healthy · M faulty · T total", tap for the per-fault-type breakdown) answers "have we collected enough yet" without guessing. **Export dataset (.zip)** zips the whole corpus (confirmed clips and the still-pending queue) and hands it straight to Android's share sheet — WhatsApp, Gmail, Drive, or any file manager via the public Downloads folder, no extra steps either way (`DatasetExporter.kt`). **Sync to Hugging Face** (`HfSyncActivity.kt`, `HfSync.kt`) pushes the same zip in one tap — not straight to Hugging Face, but through **Injini Relay** (`hf_space/`, a small backend deployed as a Hugging Face Space), which is the only thing that ever holds a real Hugging Face write token. The phone holds a separate, low-privilege upload key instead: if it leaks, the most it allows is opening a pull request against the dataset, never writing to it directly, since the relay always stages uploads as a PR for a human to review before anything reaches the branch a training run would read. Purely user-triggered; the app is offline-only otherwise. See [ADR-18](ADR.md#adr-18-dataset-export-a-live-label-counter-and-hugging-face-as-the-sync-target) for the Drive-vs-HF reasoning and [ADR-19](ADR.md#adr-19-adr-18s-direct-to-hf-sync-was-corrected--a-phone-cant-safely-hold-a-write-token) for why a direct phone-to-Hugging-Face token was wrong and this relay replaced it. `hf_space/README.md` has the deploy steps.
